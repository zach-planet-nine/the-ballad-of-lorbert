...
public class BattleManager : MonoBehaviour
{
    ...
    public int EntityUsesGasParticlesOnEntity(GameObject attacker, GameObject defender)
    {
        BattleStats attackerStats = GetStatsForEntity(attacker);
        BattleStats defenderStats = GetStatsForEntity(defender);

        int damage = attackerStats.wisdom * 2 + Randomness.GetIntBetween(0, attackerStats.luck);
        damage -= defenderStats.aura + Randomness.GetIntBetween(0, defenderStats.luck);

        return damage;
    }

    public int EntityHomingMissilesEntity(GameObject attacker, GameObject defender)
    {
        BattleStats attackerStats = GetStatsForEntity(attacker);
        BattleStats defenderStats = GetStatsForEntity(defender);

        int damage = attackerStats.agility * 2 + attackerStats.perception + Randomness.GetIntBetween(0, attackerStats.luck);
        damage -= defenderStats.dexterity + defenderStats.vitality + Randomness.GetIntBetween(0, defenderStats.luck);
